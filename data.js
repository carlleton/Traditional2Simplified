...
var traditionalWords = ['週','佈','元件','模彷','鬆','複','桉','裡','係','昇','徵','籤','馀','溷','讚','製','採','併','麽','樑','着作','名着','编着','着名','软体']
var simplifiedWords = ['周','布','组件','模仿','松','复','案','里','系','升','征','签','余','混','赞','制','采','并','么','梁','著作','名著','编著','著名','软件']
...
function simplized(cc) {
  var str='';
  var simpleStr = charPYStr()
  var ftStr = ftPYStr()
  for (var i = 0; i < cc.length; i++) {
    if (ftStr.indexOf(cc.charAt(i)) != -1) {
      str += simpleStr.charAt(ftStr.indexOf(cc.charAt(i)));
    } else {
      str += cc.charAt(i);
    }
  }
  var arr_simple = simpleStr.split('').concat(simplifiedWords)
  var arr_ft = ftStr.split('').concat(traditionalWords)
  for (var i = 0, n = arr_ft.length; i < n; i++) {
    var reg=new RegExp(arr_ft[i], "g");
    str = str.replace(reg, arr_simple[i])
  }
  return str;
}
...
/**
 * 转换繁简体
 * @param  nOption 类型，0繁转简，1简转繁，2转qq繁体
 * @param  str
 * @return 
 */
function convert(nOption, str){
  if (nOption==0) {
